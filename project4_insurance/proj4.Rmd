...
## Classification Models: Models 1, 2, 3, 4

The first four models take categorical variables as inputs and interpret their contributions to predicting the likelihood of a claim. We use `drop` and `MASS:stepAIC` functions to judge which variables to remove, evaluating AIC statistics as we go.

### Model 1

> TARGET_FLAG ~ NumParents + Male + EDUCATION + JOB + CAR_TYPE + RED_CAR + REVOKED + Urban + Single + Commercial

For an easily interpretable model aimed at predicting TARGET_FLAG, inputs for Model 1 were restricted to categorical variables alone. The AIC metric suggests that the `RED_CAR` variable can be removed.
...
```{r f11, fig.cap="Model 1 Diagnostic Plots", fig.height=6}
par(mfrow=c(2,2)); plot(mod.1)
```
...
> TARGET_FLAG ~ KIDSDRIV + AGE + HOMEKIDS + YOJ + INCOME + HOME_VAL + TRAVTIME + BLUEBOOK + TIF + OLDCLAIM + CLM_FREQ + MVR_PTS + CAR_AGE + PARENT1 + SEX + EDUCATION + JOB + CAR_TYPE + REVOKED + URBANICITY + MSTATUS + CAR_USE
...
```{r f12, fig.cap="Model 2 Diagnostic Plots", fig.height=6}
par(mfrow=c(2,2)); plot(mod.2)
```
...
> TARGET_FLAG ~ KIDSDRIV + HOMEKIDS + YOJ + INCOME + HOME_VAL + TRAVTIME + BLUEBOOK + TIF + OLDCLAIM + CLM_FREQ + MVR_PTS + PARENT1 + EDUCATION + JOB + CAR_TYPE + REVOKED+ URBANICITY+ MSTATUS+ CAR_USE
...
```{r f13, fig.cap="Model 3 Diagnostic Plots", fig.height=6}
...
```{r f14, fig.cap="Model 4 Diagnostic Plots", fig.height=6}
par(mfrow=c(2,2)); plot(model.4)
```
...
A new variable `milage` was created based on `TRAVTIME` and `CAR_AGE`.
...
par(mfrow=c(2,2)); plot(forward.mod.5)
```
...
par(mfrow=c(2,2)); plot(forward.mod.6)
```
