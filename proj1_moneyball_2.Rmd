...
qqplot_exp <- ggplot(data = imputed_train, aes(sample = TARGET_WINS)) +
  ...
  labs(title = 'Model 1: Quartile-Quartile Plot', 
...
Our approach for Model 2 was to try to use as many of the tools available in R that we have learned ths far as possible to determine a model based solely on the statistical significance of the predictor variables without any regard to our expert's opinion.
...
Based on the charts above we decided somewhat arbitrarily to remove the three pitching variables (`PITCHING_HR`, `PITCHING_BB`, and `PITCHING_SO`) rather than the corresponding batting variables (`BATTING_HR`,  `BATTING_BB`, and `BATTING_SO`) due to the extremely high correlation between thesee predictor variables.  

```{r, fig.height=8, fig.cap = "Each Predictor vs. Target"}
...
```{r, fig.height=8, fig.cap = "Log Transformed Predictors vs. Target"}
lm_data %>%
  gather(-TARGET_WINS, key = "var", value = "value") %>%
  ggplot(aes(x = value, y = TARGET_WINS)) +
    geom_point(alpha=0.1) +
...
qqplot_exp <- ggplot(data = lm_data, aes(sample = TARGET_WINS)) +
  stat_qq(size = 1.5) +
  stat_qq_line(color = 'darkgray') +
  labs(title = 'Model 2: Quartile-Quartile Plot', 
       x = "Theoretical Quantiles", 
       y = "Standardized Residuals") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = .5))
...
```{r}
predictions <- round(predict(lm1, log_test))
```
...
```{r}
lm1 <- lm(TARGET_WINS ~ ., imputed_train)
...
```{r}
#removed PITCHING_SO + BATTING_H and changed to BATTING_BB*PITCHING_BB
...
logtransform_qqplot <- ggplot(logtransform_lm, aes(sample = .stdresid)) +  # for each model update chart object name and model object 
  stat_qq(size = 1.5) +
  stat_qq_line(color = 'darkgray') +
  labs(title = 'Model 3: Quartile-Quartile Plot', 
       x = "Theoretical Quantiles", 
       y = "Standardized Residuals") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = .5))
