...
We removed 23 lines where `TARGET_AMT` greater than $45,000, these lines had `BLUEBOOK` value much less than car crash cost. 
New variable `milage` was created based on `TRAVTIME` and `CAR_AGE`.

### Model 5 - Multiple linear regression model

Model 5 is a multiple linear regression model built only on cases with claims where `TARGET_FlAG` is equals 1. The model is refined using stepwise elimination. From the model summary we can see that the Adjusted R-squared value is very low at 0.04.
...
Model 6 includes is a multiple linear regression model built on all cases - in other words it relaxes the constraint that a claim was filed, and so includes `TARGET_AMT` values of 0. Forward elimination is used to refine variable selection. The Adjusted R-squared value significantly improved compare to the previous model. 
