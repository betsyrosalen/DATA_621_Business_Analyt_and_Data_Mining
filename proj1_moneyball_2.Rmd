...
## Instructions: (Betsy: we can take these out later, but it helps me to see them right here for now...)
...
There are also a large number of outliers that may need to be handled...  (need to write more here...)

```{r}
melt.train <- melt(train)
# Boxplot
ggplot(melt.train, aes(variable, value)) + 
    geom_boxplot(width=.5, fill="#58BFFF", outlier.colour="#58BFFF") +
    scale_y_log10() + 
    ...
    labs(colour="Statistics", title="Boxplot",
        x="", y="log transformed freq.") + 
...
kable(logtransform_lm_summary$coef[,1], caption = "Log Transform Model Coefficients")
